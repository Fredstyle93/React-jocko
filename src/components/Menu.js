...
class Menu extends Component {
    ...
    componentDidMount() {
        ...
        const btn = document.querySelector('button').style.color = "#fff";
    }
    ...
    getMenu() {
        ...
        return (
            <Menu burgerButtonClassName={ "btn-menu" } id={this.state.currentMenu} pageWrapId={'page-wrap'} outerContainerId={'outer-container'} right={this.state.side === 'right'}>
                <h4 className="userName"><FaUser className="icon-menu"/>{firebase.auth().currentUser !== null ? this.state.userInfo[6] : ""} <span className="level-show">{this.state.userInfo[2]}</span></h4>
                {this.getItems()}
            </Menu>
        );
    }
